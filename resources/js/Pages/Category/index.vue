    ...
    <a-layout-content
      ...
      >
       ...
       <a-form name="customized_form_controls" layout="inline" :model="formState" @finish="search">
          <a-form-item>
            <a-input v-model:value="formState.term"  placeholder="Search Category Name"></a-input>
          </a-form-item>
          <a-form-item>
        ...
        <a-table :columns="columns" :row-key="category => category.id" :data-source="categories.data"
          ...
          <template #bodyCell="{ column, text, record }">
            ...
            <template v-if="column.key === 'action'">
              <a-dropdown>
                <template #overlay>
                  <a-menu>
                    <Link :href="route('category.edit', record.id)">
                    <a-menu-item key="1">
                      Edit Category
                    </a-menu-item>
                    </Link>
                    <a-popconfirm title="Are you sure？" @confirm="deleteCategory(record.id)">
...
export default {
  ...
  methods: {
    search() {
      this.$inertia.get("/category", { s: this.formState.term }, { preserveState: true });
    },
