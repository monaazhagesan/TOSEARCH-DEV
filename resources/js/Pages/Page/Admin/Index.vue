    ...
    <a-layout-content
      ...
      >
       ...
       <a-form name="customized_form_controls" layout="inline" :model="formState" @finish="search">
          <a-form-item>
            <a-input v-model:value="formState.term"  placeholder="Search Pages"></a-input>
          </a-form-item>
          <a-form-item>
        ...
        <a-table :columns="columns" :row-key="page => page.id" :data-source="pages.data"
          ...
          <template #bodyCell="{ column, text, record }">
            <template v-if="column.key === 'action'">
              <a-dropdown>
                <template #overlay>
                  <a-menu>
                    <Link :href="route('pages.edit', record.id)">
                    <a-menu-item key="1">
                      Edit Pages
                    </a-menu-item>
                    </Link>
...
export default {
  ...
  methods: {
    search() {
      this.$inertia.get("/pages", { s: this.formState.term }, { preserveState: true });
    },
    ...
    deletePages(id) {
      this.$inertia.delete(route("pages.destroy", id));
    },
  }
};
</script>
